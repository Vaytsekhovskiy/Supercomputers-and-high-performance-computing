...
/*
    9_3_packed.c
    Версия с упакованными сообщениями (MPI_Pack / MPI_Unpack).

    Идея:
    - Вместо "сырых" массивов отправляем единый упакованный буфер.
    - На приёме распаковываем в том же порядке.
    - Добавлен замер времени MPI_Wtime.
    - Печатаем Tp, ускорение и эффективность (если передан T1 базовой версии).

    Запуск:
      mpiexec -n 4 .\9_3_packed.exe
      mpiexec -n 4 .\9_3_packed.exe <T1_plain>

    Где T1_plain — время baseline-версии 9_3 (без Pack), чтобы сравнить:
      S = T1_plain / Tp_packed
      E = S / P
*/
