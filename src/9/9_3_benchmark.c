...
/*
Этот бенчмарк измеряет производительность именно программы 9_3,
где межбанковские расчёты идут через Большой Банк.
Каждый процесс обрабатывает свою часть расписок, затем отправляет частичный результат в центральный процесс (rank 0).
Центральный процесс агрегирует данные, считает чистую позицию каждого банка по каждой валюте (сколько получить/отдать)
и рассылает итог обратно. Таким образом моделируется полный цикл:
локальная обработка, point-to-point обмен, клиринг, обратная передача результата.
Время измеряется через MPI_Wtime() вокруг основного вычислительно-коммуникационного участка.
Для корректной оценки параллельного времени берётся максимум среди всех процессов,
потому что завершение всей программы определяется самым медленным процессом.
Далее при запуске с P=1 получаем T1,
при запуске с P>1 получаем Tp, после чего считаются
S = T1 / Tp
E = S / P.
Ускорение показывает, во сколько раз параллельная версия быстрее,
а эффективность — насколько хорошо используется каждый добавленный процесс.
На малых данных (small) эффективность обычно ниже из-за накладных расходов связи и синхронизации,
на более крупных (medium) — выше, потому что растёт доля полезных вычислений.
*/
...
/*
 Примеры вывода для датасета SMALL:
...
Примеры вывода для датасета MEDIUM
...
Итоговая таблица:
Dataset	    Receipts	P	    Tp (сек)	    Speedup	    Efficiency
small	    9	        1	    0.000004	    —	        —
small	    9	        10	    0.000141	    0.0285	    0.28%
small	    9	        100	    0.005230	    0.0008	    0.00%
medium	    42	        1	    0.000005	    —	        —
medium	    42	        10	    0.000272	    0.0184	    0.18%
medium	    42	        100	    0.008260	    0.0006	    0.00%
